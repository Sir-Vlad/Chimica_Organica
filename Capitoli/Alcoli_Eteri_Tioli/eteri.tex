...
Mentre, per preparare un etere asimmetrico ci sono due metodi:
\begin{enumerate}
	\item reazione tra un alchene terziario con un alcol con catalizzatore
	      \begin{reaction}
		      \AddRxnDesc{Preparazione eteri asimmetrici}
		      \chemfig{C(-[3])(-[5])(=[0]C(-[1])(-[7]))} \+ \ch{R-OH} \arrow{->[\ch[circled=all,circletype=math]{H^{\color{red}+}}]} \chemfig{C(-[2])(-[4]H)(-[6])(-[0]C(-[0]O-R)(-[2])(-[6]))}
	      \end{reaction}
	\item \textbf{Sintesi di Williamson}, nella quale viene prima creato l'alcossido tramite trattamento con sodio metallico e poi si fa reagire con un alogenuro alchilico tramite meccanismo \mech[2]
	      \begin{reactions}\label{rcn:sintesiWilliamson}
		      \AddRxnDesc{Sintesi di Williamson per la preparazione di eteri asimmetrici}
...
\begin{reaction}
	...
	\arrow(@c1.30--.180){->[*{0.south east}\chemfig{R''-OH}][*{0.north west}\ch{H^{\color{red}\oplus}}]}[45]
	\chemfig{RC(-[6,,2]OH)(-[0]CR'(-[6]OR''))}
	\arrow(@c1.-30--.180){->[*{0.south west}\chemfig{HO-R''-OH}][*{0.north east}\ch{H^{\color{red}\oplus}}]}[-45]
