...
Gli aldeidi e i chetoni sono composti caratterizzati dalla presenza del \textbf{gruppo carbonile}. Le aldeide hanno il carbonile in posizione terminale, mentre nei i chetoni si trova all'interno della catena. Il gruppo \ch{-CHO} delle aldeidi si chiama \textbf{gruppo formilico}.
...
\begin{figure}[H]
	\centering
	\schemestart
	\chemfig{@{C}C(=[2]@{O}O)(-[:210]C(-[6]@{Ha}H)(-[4]))(-[:-30])}
	\arrow(@c1--){0}[45,0.5] \chemfig{@{Hc}\charge{35:3pt=\chargeColor{+}}{H}}
	\arrow(@c1--){0}[,0.5] \chemfig{@{nu}\charge{90:2pt=\:}{Nu}}
	\arrow(@c1--){0}[-45,0.5] \chemfig{@{B}\charge{180:2pt=\:}{B}}
	\schemestop
	\chemmove[green!60!black!70]{
		\draw[shorten <=3pt, shorten >= 1pt] (O).. controls +(90:1cm) and +(180:1cm) .. (Hc);
		\draw[shorten <=5pt, shorten >= 1pt] (nu).. controls +(90:1cm) and +(45:1cm) .. (C);
		\draw[shorten <=5pt, shorten >= 1pt] (B).. controls +(180:1cm) and +(0:1cm) .. (Ha);
	}
\end{figure}
