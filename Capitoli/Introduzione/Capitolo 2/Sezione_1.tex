...
Quando un acido si scioglie in acqua, esso reagisce con l'acqua per produrre \ch{H+}/\ch{H3O+}. Per esempio, quando \ch{HCl} si scioglie in acqua reagisce con l'acqua per dare ione idronio e ione cloruro:


\begin{center}
	\ch[circled=all,circletype=math]{
	H2O_{(l)} + HCl_{(l)} -> !(idronio)( H3O^{\color{red}+}_{(aq)} ) + Cl^{\color{blue}-}_{(aq)}
	}
\end{center}
